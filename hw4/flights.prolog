...
% Find Other Paths
route(Departing, Arriving, Visited, DepartureTime, ArrivalTime, Flights, TotalCost) :-
  flight(Departing, Connecting, DepartureTime, ConnectingArrival, InitialCost),
  \+ member(Connecting, Visited), % Ensure connecting flight destination is not visited yet
  append(Visited, [Departing, Connecting], NewVisited), % Update list of visited airports
  route(Connecting, Arriving, NewVisited, ConnectingTime, ArrivalTime, FlightList, FlightCost), % Recurse
  ConnectingArrival @< ConnectingTime, % Connecting flight departure time needs to be later than the current arrival time
  Flights = [Departing|FlightList], % Update the list of flights
  TotalCost is InitialCost + FlightCost. % Update the cost
