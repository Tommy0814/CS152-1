...
CoinClient.prototype.on('proof', function(self, trans) {
  ...
  var transLength;
  valid = true;
  localLength = self.chainLength(self.ledger);
  transLength = self.chainLength(trans.ledger);
  if(localLength >= transLength)
    valid = false;
  ledgerProof = JSON.stringify(trans.ledger) + trans.proof;
  key = this.hash(ledgerProof);
  zeroes = Array(ZEROES_REQ + 1).join('0');
  if(key.substring(0, ZEROES_REQ) !== zeroes)
    valid = false;
  if(valid) {
    self.ledger = trans.ledger;
    self.log("New ledger accepted - proof " + trans.proof);
    self.showLedger();
  }
});
...
CoinClient.prototype.mineProof = function(newLedger, start) {
  ...
  zeroes = Array(ZEROES_REQ + 1).join('0');
  if (this.chainLength(this.ledger) >= this.chainLength(newLedger)) 
	  return;
  for(index = 0; ; index++) {
    proof = start + index;
    ledgerProof = ledge + proof;
    key = this.hash(ledgerProof);
    if (key.substring(0, ZEROES_REQ) === zeroes)
      break;
  }
  this.log("Proof found - " + proof)
